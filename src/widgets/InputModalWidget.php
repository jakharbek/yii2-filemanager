...
JS;
        ...
        if(strlen($this->values)){
            ...
            $files = Files::find()->where(['file_id' => $this->values_data])->all();
            foreach ($files as $file){
                $this->selected[] = $file->file_id;
                $this->selected_items_sortable[$file->file_id]['content'] = "";
                if(in_array($file->type,$this->pictures_ext))
                {
                    $this->selected_items_sortable[$file->file_id]['content'] .= "<img class='col-md-2' style='height:50px; width:100px;' src='".$file->getImageSrc()."' />";
                }
                if(in_array($file->type,$this->video_ext))
                {
                    $this->selected_items_sortable[$file->file_id]['content'] .= "<video class='col-md-2' style='height:auto; display:inline;' src='".$file->getImageSrc()."'> </video>";
                }
                $this->selected_items_sortable[$file->file_id]['content'] .= "<h5 style=''><i class='glyphicon glyphicon-trash delete_item' onclick='document.filemanager{$this->id}.deleteItem(this);'></i> ".$file->title."</h5>";
